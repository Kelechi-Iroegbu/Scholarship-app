...
const Image = React.forwardRef(
  (
    ...
    // Non-Wix images (anything parseWixMediaUrl doesn't recognize) render as a
    // plain <img> with no server-side crop, so it needs its own object-fit or
    // it stretches to fill the aspect-ratio box instead of cropping into it.
    const { className, ...restImageProps } = imageProps
    const plainImgClassName = cn(fittingType === "fit" ? "object-contain" : "object-cover", className)
    ...
    if (!src) {
      ...
      return (
        <img ref={ref} src={FALLBACK_IMAGE_URL} className={plainImgClassName} {...restImageProps} data-empty-image />
      )
    }
